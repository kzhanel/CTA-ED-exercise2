...
```{r}
anxiety_words <- c('anxiety', 'anxious', 'worry', 'worried', 'tense', 'nervous', 'uncomfortable', 'uneasy', 'unsettled', 'stress', 'restless')
...
proportions <- 
  tidy_tweets %>%
  mutate(obs=1) %>%
  filter(grepl(paste0(anxiety_words, collapse = "|"), word, ignore.case = TRUE)) %>%
  group_by(date) %>%
