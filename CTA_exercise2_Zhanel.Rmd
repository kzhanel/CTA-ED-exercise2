...
```{r}
head(tweets) ##head means table only shows snapshot of data
colnames(tweets) ##number and names of columns in dataset
```
...
tweets <- tweets %>%
  select(user_username, text, created_at, user_name,
         retweet_count, like_count, quote_count) %>% ##selects columns that i need
  rename(username = user_username,
         newspaper = user_name,
         tweet = text) ##renames columns right to left

```

```{r, echo = F}
tweets %>% 
  arrange(created_at) %>% ##orders by column created_at
  tail(5) %>% ##probs a row
  kbl() %>% ##to generate table
  kable_styling(c("striped", "hover", "condensed", "responsive")) ##kable shows all tweet description in table
```
...
```{r}
tidy_tweets <- tweets %>% 
  mutate(desc = tolower(tweet)) %>% ##mutate adds columns to the dataset and tolower changes from uppercase to lowercase
  unnest_tokens(word, desc) %>% ##splits into each single word 1 word = 1 row
  filter(str_detect(word, "[a-z]")) ##regular expr lowerc case from a to z
```
...
tidy_tweets <- tidy_tweets %>%
    filter(!word %in% stop_words$word) ##ask tomorrow to check
```
...
```{r}
get_sentiments("afinn") ##to show dictionary and their scores
```
...
nrc_fear <- get_sentiments("nrc") %>% 
  filter(sentiment == "fear") ##choosing fear only sentiments

tidy_tweets %>%
  inner_join(nrc_fear) %>% ##merges same value rows and columns together
...
tidy_tweets <- tidy_tweets %>%
  arrange(date) ##arrange the order of the dates

tidy_tweets$order <- 1:nrow(tidy_tweets) ##creating new column called order that numbers each row
...
tweets_nrc_sentiment <- tidy_tweets %>%
  ...
  mutate(sentiment = positive - negative) ##creates new column for sentiment score

tweets_nrc_sentiment %>%
  ggplot(aes(date, sentiment)) + ##ggplot creates graph; aes means what goes into the graph
  geom_point(alpha=0.5) + ##geom point adds layer scatterplot to graph
  geom_smooth(method= loess, alpha=0.25) ##geom smooth adds trend line
...
mordict <- data.frame(word, value) ##makes in into a dataset
mordict
...
tidy_tweets %>%
  ...
  geom_bar(stat= "identity") + ## adjusts the height of bars = values of the data
  ylab("mortality words") ##naming y axis
```
...
totals <- tidy_tweets %>%
  mutate(obs=1) %>% ##add column obs=1
  group_by(date) %>%
  summarise(sum_words = sum(obs)) ##create new column sum_words adding all the numbers in obs column

#plot
tidy_tweets %>%
  mutate(obs=1) %>%
  filter(grepl(paste0(mordict, collapse = "|"),word, ignore.case = T)) %>% ##paste0 adds separation sign |; grepl matches words with similar patterns; ignore.case is a new V
  group_by(date) %>%
  summarise(sum_mwords = sum(obs)) %>%
  full_join(totals, word, by="date") %>% ##matching relevant values in both rows and columns combined
  mutate(sum_mwords= ifelse(is.na(sum_mwords), 0, sum_mwords), ##is.na looks for missing values in a column 
         pctmwords = sum_mwords/sum_words) %>% ##ifelse creates condition ##percentage of missing words
...
grid()
abline(h = 0, lty = 2) ##adds vertical, horiz or regression lines to graph
...
Question 1 
Looking at "fear" sentiment still keeping the same sentiment from above

Go through same process from code row 22-186
```{r}
#get tweet sentiment example by newspaper (nrc)
tweets_nrc_sentiment <- tidy_tweets %>%
  inner_join(get_sentiments("nrc")) %>% ##inner_join means finding a matching value within both column and row if any words within both nrc and tidy_tweets match
  count(newspaper, index = order %/% 1000, sentiment) %>%
  spread(sentiment, n, fill = 0) %>% ##we only consider those words that reach 1000 mentions and then we calculate net sentiment score which is sentiment column. spread means changing every sentiment cell to a column name
  mutate(sentiment = positive - negative) #separate column with a score
...
```{r}
#get tweet sentiment by newspaper (all dictionaries)
tidy_tweets %>%
  inner_join(get_sentiments("bing")) %>%
  count(newspaper, index = order %/% 1000, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
  ...
  geom_smooth(method= loess, alpha=0.25) +
  ylab("bing sentiment") +
  theme(axis.text.x = element_text(angle = 45)) ##theme deals with format of graph in general. if i change text formatting in x axis i put axis.text.x. element_text means the layout of the text within a graph
...
tidy_tweets %>%
  inner_join(get_sentiments("afinn")) %>%
  group_by(newspaper, index = order %/% 1000) %>% 
  summarise(sentiment = sum(value)) %>% 
  ggplot(aes(newspaper, sentiment)) +
  geom_point(alpha=0.5) +
  geom_smooth(method= loess, alpha=0.25) +
